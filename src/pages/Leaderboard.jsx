    ...
    <div>
      ...
      <div className="p-8 bg-gradient-to-br from-blue-100 to-blue-300 min-h-screen">
        ...
        <div className="flex flex-col items-center"> {/* Center the items */}
          {quizzes.length > 0 ? (
            quizzes.map((quiz) => (
              <div key={quiz.id} className="bg-white w-2/4 p-6 rounded-lg shadow-md mb-4 transition-transform transform">
                <div className="flex justify-between items-center">
                  ...
                  >
                    <FaChevronDown className={`${expandedQuiz === quiz.id ? 'transform rotate-180' : ''}`} />
                  </button>
                </div>
                <div
                  style={{
                    maxHeight: expandedQuiz === quiz.id ? '300px' : '0',
                    overflow: 'hidden',
                    transition: 'max-height 0.3s ease-in-out', // Smooth transition for height
                  }}
                >
                  {expandedQuiz === quiz.id && leaderboards[quiz.id] && (
                    <ul className="mt-4">
                      {leaderboards[quiz.id].map((entry, index) => (
